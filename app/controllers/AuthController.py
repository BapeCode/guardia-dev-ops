...
class AuthController:
    ...
    def login():
        if request.method == "POST":
            ...
            user: 'User' = User.query.filter_by(email=dto.email, password=dto.password).first()
            if not user:
    ...
    def register():
        if request.method == "POST":
            ...
            current_user = User.query.filter_by(email=dto.email, password=dto.password).first()
            if current_user:
