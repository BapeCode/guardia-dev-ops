...
from ..controllers.PostControllers import PostControllers
from ..decorators.auth import jwt_user
...
@jwt_user
def route_posts(current_user):
    return PostControllers.index(current_user)

@post_bp.route("/posts/create", methods=["POST"])
@jwt_user
def route_posts_create(current_user):
    return PostControllers.create(current_user)

@post_bp.route("/posts/<int:id>/delete", methods=["POST"])
@jwt_user
def route_posts_delete(current_user, post_id):
    return PostControllers.delete(current_user, post_id)
